...
Chaque instance TockDocs inclut un serveur MCP intégré, préparant votre contenu pour l'écosystème IA plus large où n'importe quel client MCP (comme Claude, Cursor, VS Code, et autres) peut se connecter à votre documentation.
...
TockDocs fournit trois outils par défaut qui permettent à n'importe quel LLM de découvrir, rechercher et lire votre documentation :

### `search-pages`

Recherche dans les titres, descriptions, sections, chemins et le corps complet des pages. C'est le meilleur premier outil pour les questions factuelles, le dépannage et les recherches de configuration.

| Paramètre | Type               | Description                                         |
| --------- | ------------------ | --------------------------------------------------- |
| `query`   | string (requis)    | Requête de recherche ou question en langage naturel |
| `limit`   | number (optionnel) | Nombre maximal de résultats à retourner             |
| `locale`  | string (optionnel) | Filtrer les résultats par locale                    |

`search-pages` utilise FlexSearch pour le passage principal et Fuse.js comme repli flou, ce qui le rend plus tolérant aux petites fautes de frappe.

### `list-pages`

Liste toutes les pages de documentation avec leurs titres, chemins et descriptions. Utilisez-le lorsque le modèle doit parcourir la structure des docs par section ou nom de page.
...
| Paramètre | Type            | Description                                                                 |
| --------- | --------------- | --------------------------------------------------------------------------- |
| `path`    | string (requis) | Le chemin de la page (ex: `/docs/platform/fr/getting-started/installation`) |

## Configuration

Le serveur MCP TockDocs utilise le transport HTTP et peut être installé dans différents assistants IA.
...
Puisque TockDocs utilise le module `@nuxtjs/mcp-toolkit`, vous pouvez étendre le serveur MCP avec des outils, ressources, prompts et handlers personnalisés.
...
Vous pouvez remplacer les outils par défaut `search-pages`, `list-pages` ou `get-page` en créant un outil avec le même nom dans votre projet :
