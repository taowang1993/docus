...
description: TockDocs génère des fichiers de contenu prêts pour l'IA grâce au module Nuxt LLMs
navigation:
  icon: i-lucide-message-circle-code
---

TockDocs intègre `nuxt-llms` par défaut pour préparer votre contenu aux Large Language Models (LLMs). Toutes vos pages de documentation sont injectées et les fichiers `/llms.txt` et `/llms-full.txt` sont automatiquement générés et pré-rendus.

::note{to="/llms.txt"}
Consultez le fichier `/llms.txt` généré pour la documentation TockDocs elle-même.
::
...
::note
Exemple de chemin Markdown brut pour cette page : `/raw/docs/platform/fr/ai/llms.md`
::
...
::note
Cette fonctionnalité n'est disponible que lorsque TockDocs est déployé sur Vercel. Nous pourrons la rendre agnostique une fois que Nitro v3 prendra en charge les réécritures globales pour plusieurs fournisseurs.
::

Lorsqu'il est déployé sur Vercel, TockDocs configure automatiquement un routage intelligent pour servir du contenu markdown aux agents IA et aux outils en ligne de commande.
...
TockDocs détecte les requêtes provenant d'agents IA et d'outils en ligne de commande à l'aide des en-têtes HTTP :
...
curl -H "Accept: text/markdown" https://tockdocs.dev/fr

# Obtenir le markdown brut d'une page de documentation
curl -H "Accept: text/markdown" https://tockdocs.dev/docs/platform/fr/ai/llms
```
