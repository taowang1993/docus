...
TockDocs vous permet d'écrire tout votre contenu en **Markdown** mais offre aussi la possibilité d'intégrer des **composants** grâce à la [syntaxe MDC](https://content.nuxt.com/docs/files/markdown#mdc-syntax) fournie par **Nuxt Content**.

::tip{to="/docs/platform/fr/essentials/components"}
Consultez la liste des composants prose Nuxt UI que vous pouvez intégrer dans votre Markdown.
::

## Page d'accueil

La page d'accueil est la première page que vos visiteurs voient à la racine `/` de votre site. Par défaut, TockDocs utilise le fichier `content/index.md` pour afficher cette page.
...
Lorsqu'aucune page d'accueil personnalisée n'existe, TockDocs automatiquement:
...
**Lorsque vous créez** `app/pages/index.vue`, TockDocs :
...
Pour commencer, éditez ou ajoutez simplement des fichiers `.md` dans le dossier `content/` pour mettre à jour vos pages. TockDocs gère automatiquement le routage, la navigation et la recherche plein texte.
...
Vous pouvez optionnellement organiser vos fichiers de documentation dans un sous-dossier `docs/`. Lorsque TockDocs détecte un dossier `docs/` dans votre répertoire `content/`, il préfixe automatiquement toutes les URLs de documentation avec `/docs`.
...
::tip
Ceci est particulièrement utile lorsque vous souhaitez utiliser TockDocs comme documentation intégrée aux côtés d'autres pages personnalisées. Vous pouvez créer des pages supplémentaires comme un blog, une page de contact, une page de tarification, ou tout autre contenu personnalisé au niveau racine, tout en gardant votre documentation organisée sous `/docs`.
::

**Exemple avec contenu mixte :**

Puisque TockDocs est un layer Nuxt, vous pouvez combiner la documentation Markdown avec des pages Vue personnalisées :
...
Cette structure vous donne la flexibilité de construire un site web complet avec TockDocs. Utilisez Markdown pour la documentation et des pages Vue pour les fonctionnalités personnalisées comme les blogs, tableaux de bord, ou toute page interactive.
