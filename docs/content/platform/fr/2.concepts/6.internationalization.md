...
TockDocs introduit un **support d'internationalisation natif** basé sur le module `@nuxtjs/i18n`, vous permettant de créer une documentation en plusieurs langues avec un routage automatique et une gestion de contenu.

## Fonctionnalités

- **Module i18n intégré** : Intégration native avec `@nuxtjs/i18n`
- **Routage de locale dynamique** : URLs de locale orientées KB comme `/docs/platform/en` et `/docs/platform/fr`
...
Pour activer l'i18n dans votre projet TockDocs, ajoutez le module `@nuxtjs/i18n` à votre `nuxt.config.ts` et définissez vos locales :
...
::warning
TockDocs surcharge la stratégie `@nuxtjs/i18n` en `prefix`.
::
