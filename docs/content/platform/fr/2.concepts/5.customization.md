...
description: Découvrez comment surcharger les composants intégrés dans TockDocs pour personnaliser votre documentation.
navigation:
  icon: i-lucide-pen-tool
---

TockDocs est construit sur Nuxt 4 qui offre un système de couches de composants flexible permettant de surcharger certaines parties de l'UI en redéfinissant des composants spécifiques dans votre propre application. Cela facilite la personnalisation complète de l'apparence visuelle et du comportement de votre documentation sans toucher au thème principal.

Pour surcharger un composant, créez simplement un fichier Vue du même nom dans le dossier `components/`. TockDocs utilisera automatiquement votre version à la place de celle par défaut.
...
to: https://github.com/taowang1993/tockdocs/blob/app/components/app/AppHeaderLogo.vue
variant: link
...
## to: https://github.com/taowang1993/tockdocs/blob/docs/components/AppHeaderCTA.vue

Par défaut, ce composant est vide mais vous pouvez voir comment nous le surchargeons sur la documentation TockDocs elle-même.
::

### `AppHeaderCenter`

Pour personnaliser la zone centrale du header, surchargez le composant `components/AppHeaderCenter.vue`. Votre composant remplacera la barre de recherche fournie par le thème TockDocs.
...
color: neutral
icon: i-lucide-code-xml
to: https://github.com/taowang1993/tockdocs/blob/app/components/app/AppHeaderCenter.vue
variant: link
...
color: neutral
icon: i-lucide-code-xml
to: https://github.com/taowang1993/tockdocs/blob/app/components/app/AppHeaderBody.vue
variant: link
...
Lorsque [`navigation.sub`](/docs/platform/fr/concepts/configuration#sous-navigation) est défini sur `'header'`, TockDocs affiche une barre d'onglets secondaire sous l'en-tête. Pour ajouter du contenu personnalisé à droite de cette barre (ex. un toggle, un badge), créez un composant `components/AppHeaderBottomRight.vue`.
...
Pour remplacer le côté gauche du footer, créez le fichier `components/AppFooterLeft.vue`. Votre composant remplacera celui par défaut fourni par le thème TockDocs.
...
Dans la partie droite du header de votre page de documentation, le comportement par défaut de TockDocs est d'afficher un menu déroulant avec des actions rapides liées à la source Markdown de la page courante. Cela permet au lecteur de :
...
color: neutral
icon: i-lucide-code-xml
to: https://github.com/taowang1993/tockdocs/blob/app/components/docs/DocsHeaderRight.vue
variant: link
...
Pour personnaliser la partie basse de l'aside droit des pages de documentation, créez le composant `components/DocsAsideRightBottom.vue`. Votre composant remplacera la table des matières basse fournie par le thème TockDocs.
...
color: neutral
icon: i-lucide-code-xml
to: https://github.com/taowang1993/tockdocs/blob/app/components/docs/DocsAsideRightBottom.vue
variant: link
...
TockDocs utilise [Nuxt Icon](https://github.com/nuxt/icon) avec le fournisseur [Iconify](https://iconify.design/), vous donnant accès à des milliers d'icônes prêtes à l'emploi (ex. `i-lucide-arrow-right`, `i-simple-icons-github`).
