...
When [`navigation.sub`](/docs/platform/en/concepts/configuration#sub-navigation) is set to `'header'`, Docus renders a secondary tab bar below the header. To add custom content on the right side of this bar (e.g. a toggle, a badge), create a `components/AppHeaderBottomRight.vue` component.
