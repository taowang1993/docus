...
TockDocs provides customization options to suit your needs.
...
Since TockDocs is a layer, it allows you to fully customize your landing page by creating a Vue page at `app/pages/index.vue`. This gives you full control with Vue components, custom layouts, and advanced interactions.

In this case, TockDocs:
...
To get started, simply edit or add a `.md` files in the `content/` directory to have your pages updated. TockDocs will handle routing, navigation, and full-text search automatically.
...
You can optionally organize your documentation files within a `docs/` subfolder. When TockDocs detects a `docs/` folder in your `content/` directory, it automatically prefixes all documentation URLs with `/docs`.
...
::tip
This is particularly useful when you want to use TockDocs as embedded documentation alongside other custom pages. You can create additional pages like a blog, contact page, pricing page, or any other custom content at the root level, while keeping your documentation organized under `/docs`.
::

### Mixed content

Since TockDocs is a Nuxt layer, you can combine Markdown files with custom Vue pages:
...
This structure gives you the flexibility to build a complete website with TockDocs. Use Markdown for documentation and Vue pages for custom functionality like blogs, dashboards, or any interactive pages.
...
| `layout`      | `string`                                 | Change Nuxt layout of the page (default to `docs` defined by TockDocs).                                                       |     |
