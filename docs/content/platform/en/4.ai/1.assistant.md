...
All UI texts are automatically translated based on the user's locale. TockDocs includes built-in translations for English and French.
...
Configure advanced options in `nuxt.config.ts` under `tockdocs.assistant`.
...
By default, the assistant uses TockDocs's built-in MCP server at `/mcp`:
...
By default, TockDocs prefers Vercel AI Gateway when its credentials are available. You can override the provider explicitly:
