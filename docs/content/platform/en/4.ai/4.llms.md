...
::note{to="https://tockdocs.dev/llms.txt"}
...
When `nuxt-llms` is enabled, TockDocs also exposes a raw markdown endpoint so AI agents can fetch LLM-ready source files without going through the full rendering pipeline. This reduces token usage and improves response speed for AI-powered tools consuming your documentation.
...
::note
Example raw Markdown path for this page: `/raw/docs/platform/en/ai/llms.md`
::
...
::note
This feature is only available when TockDocs is deployed on Vercel. We'll be able to make it agnostic once Nitro v3 supports global rewrites for multi vendors.
::
...
curl -H "Accept: text/markdown" https://tockdocs.dev/docs/platform/en/ai/llms
```
