...
Every TockDocs instance includes a built-in MCP server, preparing your content for the broader AI ecosystem where any MCP client (like Claude, Cursor, VS Code, and others) can connect to your documentation.
...
TockDocs provides three tools out of the box that allow any LLM to discover, search, and read your documentation:

### `search-pages`

Searches across titles, descriptions, headings, paths, and page body content. This is the best first tool for factual questions, troubleshooting, and configuration lookups.
...
`search-pages` uses FlexSearch for the primary pass and Fuse.js as a fuzzy fallback, making it more tolerant of small typos.

### `list-pages`

Lists all documentation pages with their titles, paths, and descriptions. Use this when the model needs to browse the docs structure by section or page name.
...
| Parameter | Type              | Description                                                            |
| --------- | ----------------- | ---------------------------------------------------------------------- |
| `path`    | string (required) | The page path (e.g., `/docs/platform/en/getting-started/installation`) |

## Setup

The TockDocs MCP server uses HTTP transport and can be installed in different AI assistants.
...
Since TockDocs uses the `@nuxtjs/mcp-toolkit` module, you can extend the MCP server with custom tools, resources, prompts, and handlers.
...
You can override the default `search-pages`, `list-pages`, or `get-page` tools by creating a tool with the same name in your project:
