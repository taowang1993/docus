...
- Prefer static or prerendered images, especially for OG cards and screenshots generated by `nuxt-og-image`.
- Avoid browser-screenshot image routes or other runtime image generation on Vercel unless they are absolutely necessary.
- Keep large raster assets out of Vercel runtime functions; host heavyweight reusable images on S3 or another CDN and reference them by URL instead of committing them to `public/`.
- If an asset must stay in `public/`, keep it at display size and prefer compressed WebP/AVIF over source PNG/JPEG screenshots.
...
TockDocs can also organize docs as multiple knowledge bases. When it finds one or more `content/<kb>/kb.yml` files, it switches to KB mode and serves routes from `/docs/<kb>/<locale>/...`.
